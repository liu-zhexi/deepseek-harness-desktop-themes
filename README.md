...
所有设置在“设置 → 桌面外观”页实时预览，并通过宿主设置存储 + IndexedDB 可靠持久化。
...
```bash
npm pack            # 产出 dsh-desktop-themes-1.1.0.tgz
```
...
- `tsc --noEmit`：0 错误
- 单元测试：**73 通过 / 0 失败**（覆盖默认值、读取/保存、非法回退、v1→v2 迁移、六主题 token、明暗主题、自定义主题、字体回退、壁纸校验、粒子数量档位、配色生成与对比度、毛玻璃降级、导入导出、schema 解析、样式清理）
...
│   ├── client/
│   │   ├── index.tsx          # 客户端入口（主题/外观/粒子/壁纸/清理）
