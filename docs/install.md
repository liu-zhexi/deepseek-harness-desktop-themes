...
它会生成类似 `1.6.0-dev.20260816093000` 的唯一安装版本，安装到 `web` profile 后逐一比较 `lib/client.js` 与 `lib/index.js` 的 SHA-256，并在 Windows 上重启通过 `127.0.0.1:3080` 健康检查确认的 DSH 进程。设置页“桌面小人”分组底部会显示源码版本与本次构建编号。
