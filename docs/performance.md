...
|---|---|---|---|
| 省电 `power-saver` | 20–35 | 1 | 连线、鼠标跟随、视差、光标柔光、复杂模糊 |
| 均衡 `balanced`（默认） | 40–70 | 1.5 | — |
| 高质量 `quality` | 80–120 | 2 | —（设置页提示 GPU 占用） |
...
- 设备像素比按性能档位钳制（1 / 1.5 / 2），避免高分屏过度渲染。
- 窗口尺寸变化 150ms 防抖重算。
- 主题/特效切换复用同一个 Canvas 与光晕层，不重复创建。
